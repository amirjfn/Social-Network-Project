...
class PostDeleteView(LoginRequiredMixin,View):
    def get(self, request , post_id):
        ...
        if post.user.id == request.user.id:
            post.delete()
            messages.success(request, 'پست شما با موفقیت پاک شد' , 'success')
        else:
            messages.error(request, 'شما نمیتوانید این پست را پاک کنید' , 'danger')
...
class PostUpdateView(LoginRequiredMixin , View):
    ...
    def dispatch(self, request , *args , **kwargs):
        ...
        if not post.user.id == request.user.id:
            messages.error(request, 'شما نمیتوانید این پست را تغییر دهید' , 'danger')
            return redirect('home:home')
    ...
    def post(self , request , *args , **kwargs):
        ...
        if form.is_valid():
            ...
            new_post.save()
            messages.success(request, 'پست شما با موفقیت عوض شد' , 'success')
...
class PostLikeView(LoginRequiredMixin ,View):
    def get(self, requset ,post_id):
        post = get_object_or_404(Post , id = post_id)
        like = Vote.objects.filter(post=post , user = requset.user)
        if like.exists():
            messages.error(requset, 'شما این پست را قبلا لایک کردید', 'danger')
        else:
            Vote.objects.create(post =post , user=requset.user)
            messages.success(requset, 'پست با موفقیت لایک شد', 'success')
        return redirect('home:post_detail' ,post.id , post.slug)